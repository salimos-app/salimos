 ...
 * sentido inverso para quedarse solo con la franja "sunset → dusk → night"
 * (amarillo/naranja → coral/magenta → violeta → azul noche) — la misma
 * paleta que el isotipo de la marca (assets/isotipo.png) recorre de forma
 * natural, porque plasma completo va de índigo oscuro a amarillo brillante
 * pasando exactamente por esos tonos.
 *
 ...
 * `NIGHT_U` es el extremo más oscuro real de plasma (índigo), usado tal
 * cual como "noche".
 */
const SUNSET_U = 0.86;
const NIGHT_U = 0;
...
/** Punto "crudo" de la curva en `t` ∈ [0,1] (0 = sunset, 1 = night), sin reparametrizar. */
function rawColorAt(t: number): { l: number; a: number; b: number } {
  const u = SUNSET_U + (NIGHT_U - SUNSET_U) * Math.min(1, Math.max(0, t));
  return sampleLut(u);
}

/**
 * Tabla de longitud de arco: muestrea la curva densamente y acumula la
 * distancia euclídea en OKLab (proxy razonable de distancia perceptual)
 * para poder reparametrizar "t uniforme en distancia percibida" en vez de
 * "t uniforme en el índice del LUT".
 */
const ARC_SAMPLES = 512;
const arcLengthTable: number[] = [0];
const sampleParams: number[] = [0];
{
  let prev = rawColorAt(0);
  for (let i = 1; i <= ARC_SAMPLES; i++) {
    const t = i / ARC_SAMPLES;
    const point = rawColorAt(t);
 ...
 * 1 = noche/azul-violeta), muestreado a distancia perceptual uniforme sobre
 * los datos reales de plasma. Devuelve un HEX.
 */
 ...
 * largo de la curva sunset → dusk → night (extremos incluidos).
 */
