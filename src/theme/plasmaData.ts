/**
 * Datos reales del colormap "plasma" (matplotlib), muestreados a 256
 * puntos uniformes en su parámetro [0,1]. Plasma fue diseñado por
 * Stéfan van der Walt y Nathaniel Smith optimizando percepción uniforme en
 * CAM02-UCS; los datos son de dominio público (CC0), igual que el resto de
 * la familia viridis/magma/inferno.
 *
 * `PLASMA_LUT[0]` es el extremo más oscuro (índigo, #0d0887) y
 * `PLASMA_LUT[255]` el más claro (amarillo, #f0f921). `sunsetPalette.ts`
 * recorta y recorre este LUT en sentido inverso (claro/cálido → oscuro/frío)
 * para obtener la trayectoria "sunset → dusk → night".
 */
export const PLASMA_LUT: readonly string[] = [
